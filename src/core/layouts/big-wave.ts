 ...
 * - top: a 14 px slot at the ridge with an 8 px chimney wall on the left and a 4 px one on the right. Balls skimming
 *   along the left roof at 0.2–0.6 are turned back by the left wall instead of dropping in (a bare gap or sloped lips
 *   scooped 20–100% of them at some strengths), so the slot only takes near-vertical arrivals; 0.7–1.0 skimmers hop
 *   the wall and the open slot, run down the right roof and feed the right half (tulip-r 8–10% at 0.7–0.8). A 14 px
 *   wall on both sides kept every strength below 1.0 on the left, so the right half was dead; the short right wall
 *   only trims the 1.0 stream that lands on the ridge (chucker 6% → 3.6%) without turning 0.7–0.9 back.
 * - sides: a 16 px opening in each wall (y 268–284) just above the funnel shoulder, fed by balls that have come down
 *   four pin rows beside the frame — a mixed population, which is what keeps the chucker rate smooth across strengths
 *   (0.5–2.3% at 0.4–0.8, ≤ 3.6% everywhere; the 20 px opening gave 5–6% at 0.9–1.0).
...
function warpFrame(): Segment[] {
  const l = 178, t = 128, b = 378, mx = 320, ridge = t - 14, slot = 7, chimneyL = 8, chimneyR = 4, mouth = 8;
  const left = [
    seg(l, t, mx - slot, ridge),                                                              // roof
    seg(l, t, l, 268), seg(l, 284, l, b),                                                     // side wall around the warp opening
  ...
  return [...left, seg(mx - slot, ridge, mx - slot, ridge - chimneyL), seg(mx + slot, ridge, mx + slot, ridge - chimneyR), ...left.map(mirror)];
}
...
export const BIG_WAVE_LAYOUT = buildLayout({
  ...
  windmills: [{ x: 110, y: 260, r: 10, dir: 1 }, { x: 530, y: 260, r: 10, dir: -1 }, { x: 150, y: 500, r: 10, dir: 1 }, { x: 490, y: 500, r: 10, dir: -1 }],
  catchers: [
